...
const PostDetails = () => {
  ...
  const postDelete = async () => {
    try {
      await axios.delete(URL + "/api/posts/post/" + postId).then((res) => {
        navigate("/");
      });
    } catch (err) {
      console.log(err);
  ...
  const deleteComment = async (id) => {
    try {
      await axios.delete(URL + "/api/comments/comment/" + id).then((res) => {
        fetchComments();
      });
    } catch (err) {
      console.log(err);
    ...
    <>
      ...
      {confirmDelete ? (
        ...
        <div className="px-6 md:px-[200px] mt-8">
          ...
          {!loader ? (
            <img src={photo} alt="image" className=" mx-auto mt-4" />
          ) : (
